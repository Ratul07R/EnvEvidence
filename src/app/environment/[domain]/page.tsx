...
function DomainPageInner() { 
  ...
  const [locale] = useState<Locale>(() => { 
    if (typeof window !== 'undefined') { 
      ...
      if ( 
        saved && 
        ['en', 'bn', 'zh', 'ja', 'ar', 'ru'].includes(saved) 
      ) { 
        return saved; 
      } 
    } 
  ...
  const [dataGaps, setDataGaps] = useState<DataGapView[]>([]); 
  ...
  const fetchData = useCallback(async () => { 
    ...
    try { 
      /* 
       * IMPORTANT: 
       * We now explicitly send category=<domain>. 
       * 
       * Example: 
       * /api/search?type=evidence&category=air 
       * 
       * This matches EnvironmentalCategory.slug in Prisma. 
       */ 
      const [searchRes, gapsRes] = await Promise.all([ 
        fetch( 
          `/api/search?type=evidence&category=${encodeURIComponent( 
            domain 
          )}` 
        ), 
        fetch( 
