...
function DomainPageInner() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/search?category=${encodeURIComponent(domain)}`);
      if (res.ok) {
        const json = await res.json();
        const results: SearchResult[] = json.results || [];
        // Filter for evidence type results
        setEvidence(results.filter((r) => r.type === 'evidence') as unknown as EvidenceRecord[]);
      }
    } catch {
      setEvidence([]);
    } finally {
  ...
  const meta = DOMAIN_META[domain] || { icon: Droplets, labelKey: 'topics.title', descKey: 'topics.subtitle' };
  ...
  // Group evidence by location for linking
  const evidenceByLocation = evidence.reduce<Record<string, EvidenceRecord[]>>((acc, ev) => {
    ...
    <article className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-8 md:py-12">
      <Breadcrumb className="mb-6">
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbLink asChild><Link href="/topics">{t('topics.title', locale)}</Link></BreadcrumbLink>
          </BreadcrumbItem>
          <BreadcrumbSeparator />
          <BreadcrumbItem>
      ...
      <header className="mb-8">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            <h1 className="text-2xl md:text-3xl font-bold tracking-tight">{t(meta.labelKey, locale)}</h1>
          </div>
        </div>
        <p className="text-muted-foreground mt-1">{t(meta.descKey, locale)}</p>
      </header>
        ...
        {!loading && evidence.length === 0 && (
          <Card>
            <CardContent className="p-8 text-center">
              ...
              <p className="text-sm text-muted-foreground">{t('evidence.no_evidence', locale)}</p>
            </CardContent>
          </Card>
        )}
                ...
                <Card className="transition-colors hover:border-primary/30 hover:shadow-sm">
                  <CardContent className="p-4 flex items-start gap-3">
                    <div className="min-w-0 flex-1">
                      ...
                      {ev.observationDate && (
                        <p className="text-xs text-muted-foreground mt-0.5">{ev.observationDate}</p>
                      )}
                    </div>
        ...
        <section className="mb-10">
          ...
          <div className="flex flex-wrap gap-2">
            {Object.entries(evidenceByLocation).map(([locSlug, items]) => (
              <Link
                key={locSlug}
                href={locSlug === '_none' ? '#' : `/environment/${domain}/${locSlug}`}
                className="block"
              >
        ...
        {dataGaps.length > 0 ? (
          ...
          <Card>
            <CardContent className="p-6 text-center">
              <p className="text-sm text-muted-foreground">{t('gaps.absence_note', locale)}</p>
            </CardContent>
          </Card>
        )}
      </section>
